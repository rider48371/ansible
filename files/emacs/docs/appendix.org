...
- Universal argument :: See
  ...
  #+begin_quote tip
  Doom users with evil enabled will find the universal argument on [[kbd:][SPC u]] instead
  than [[kbd:][C-u]].
  #+end_quote
...
- Prefix key :: A "prefix" is a key that begins a key sequence. For instance,
  ...
  default all [[pkg:lsp-mode]] commands are under ~SPC c l ...~, but if you want to change
  that prefix to ~SPC L ...~ for all [[pkg:lsp-mode]] commands, it is a one liner in your
  configuration; you do _not_ have to rebind each command manually to its new ~SPC
  L ...~ variant.
