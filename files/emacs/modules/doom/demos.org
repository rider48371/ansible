...
This file adds demonstrations of Doom API for the [[package:elisp-demos]] package,
installed by the [[module::lang emacs-lisp]] module, which displays these demos in
documentation buffers (produced by help.el or helpful).

The emacs-lisp module also configures elisp-demos to search for =demos.org= files
in modules and ~$DOOMDIR~.

#+begin_quote tip
New additions to this file should be sorted alphabetically and should contain a
:PROPERTIES: drawer that includes what version of Doom in which that that
...
(add-hook! '(one-mode-hook second-mode-hook) :append #'enable-something)
(add-hook! '(one-mode-hook second-mode-hook) :local #'enable-something)

;; With arbitrary forms
(add-hook! '(one-mode-hook second-mode-hook)
  (setq v 5)
  (setq a 2))
(add-hook! '(one-mode-hook second-mode-hook) :append :local
  (setq v 5)
  (setq a 2))
...
(remove-hook! 'some-mode-hook #'enable-something #'and-another)
(remove-hook! 'some-mode-hook #'(enable-something and-another))
(remove-hook! '(one-mode-hook second-mode-hook) #'enable-something)

;; Removing buffer-local hooks
(remove-hook! '(one-mode-hook second-mode-hook) :local #'enable-something)

;; Removing arbitrary forms (must be exactly the same as the definition)
(remove-hook! '(one-mode-hook second-mode-hook) (setq v 5) (setq a 2))
#+end_src
...
:END:
#+begin_src emacs-lisp :eval yes
(format "Today's date: %s" (sh< date))
#+end_src

#+RESULTS:
: Today's date: Wed Jul 15 02:04:13 AM EDT 2026

#+begin_src emacs-lisp :eval yes
(let ((buffer (get-buffer-create "x")))
...
:END:
#+begin_src emacs-lisp :eval yes
;; Assuming /tmp isn't a git repo and $EMACSDIR is
`(,(sh? git -C "/tmp" rev-parse --show-toplevel)
  ,(sh? git -C ,doom-emacs-dir rev-parse --show-toplevel))
#+end_src
...
#+begin_src emacs-lisp :eval no
(when (sh? git status --porcelain -uno)
  (message "Modifications found!"))

(when (sh? emacs --version | grep 32.0.50)
  (message "You're on Emacs 32!"))
#+end_src
